  ...
  "absolute rounded-full p-1 text-white focus:outline-none  focus:ring-2",
  {
    variants: {
      variant: {
        ...
        destructive: "bg-blue-500 hover:bg-blue-600 focus:ring-blue-300",
      },
    },
    defaultVariants: {
      variant: "default",
...
const ImageActionButton = ({ variant }: { variant: string | undefined }) => {
  return (
    <button
      className={cn(
        "absolute bottom-0 right-0 rounded-full p-1 text-white focus:outline-none focus:ring-2",
        variant
    ...
    >
      {variant ? <Trash2 className="h-4 w-4" /> : <Plus className="h-4 w-4" />}
    </button>
  );
};
